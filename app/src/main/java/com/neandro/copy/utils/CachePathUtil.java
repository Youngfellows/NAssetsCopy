...
public class CachePathUtil {
    private static String TAG = CachePathUtil.class.getSimpleName();
     ...
     * @return 返回SD卡上应用目录
     */
    public static File getExternalDirectory(Context context, String type) {
        File appCacheDir = null;
        if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())) {
            if (TextUtils.isEmpty(type)) {
                appCacheDir = new File(Environment.getExternalStorageDirectory(), context.getPackageName());
            } else {
                appCacheDir = new File(Environment.getExternalStorageDirectory(), context.getPackageName() + File.separator + type);
            }
